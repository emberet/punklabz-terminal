...
export const config = {
  ...
  /** public human/agent room; isolated from research and trading decisions */
   ...
   */
  forumHeartbeatHours: Number(process.env.FORUM_HEARTBEAT_HOURS ?? 0),
  ...
  billingProvider: (process.env.BILLING_PROVIDER ?? 'none') as 'none' | 'stripe' | 'usdg',
  billingEnforced: (process.env.BILLING_ENFORCED ?? 'false') === 'true',
  billingTreasuryAddress: (process.env.BILLING_TREASURY_ADDRESS ?? '').toLowerCase(),
  ...
  chainalysisApiUrl: process.env.CHAINALYSIS_API_URL ?? '',
  chainalysisApiKey: credential('CHAINALYSIS_API_KEY_FILE', 'CHAINALYSIS_API_KEY'),
};
...
if (config.billingProvider === 'usdg') {
  if (!/^0x[0-9a-f]{40}$/.test(config.billingTreasuryAddress)) {
    throw new Error('BILLING_PROVIDER=usdg requires a valid BILLING_TREASURY_ADDRESS');
  }
  if (!process.env.RPC_ROBINHOOD_PRIMARY) {
    throw new Error('BILLING_PROVIDER=usdg requires RPC_ROBINHOOD_PRIMARY');
  }
}
if (config.billingEnforced) {
  if (config.billingProvider === 'none') throw new Error('BILLING_ENFORCED requires a configured billing provider');
}
